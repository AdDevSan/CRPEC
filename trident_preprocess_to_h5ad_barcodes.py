...
def main(trident_directory, h5ad_directory, filtered_barcodes_directory):
    ...
    h5ad_file_path = os.path.join(h5ad_directory, f"{base_name}.h5ad")
    adata_processed.write(h5ad_file_path)

    # Write the filtered barcodes to a TSV file
    filtered_barcodes_file_path = os.path.join(filtered_barcodes_directory, f"{base_name}_filtered_barcodes.tsv")
